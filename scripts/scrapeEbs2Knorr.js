...
(async () => {
  ...
  await page.goto('https://kodtruck.ru/mank2.htm');

  const links = await page.$$eval('tbody[class="style39"] a', (links) =>
    links
      .map((link) => ({
        href: link.href,
      }))
      .filter((link) => link.href.includes('man'))
  );
  ...
  fs.writeFileSync('data/ebs2Knorr.csv', csvData, 'utf8');
